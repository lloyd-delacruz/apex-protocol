...
function CalendarView() {
  ...
  // Mock workout days (you'd replace this with real data)
  const workoutDays = new Set([3, 7, 10, 14, 17]);
...
export default function LogScreen() {
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const res = await api.request<{ logs: any[] }>('GET', '/api/training-log');
      if (res.success && res.data?.logs) {
        const logs = res.data.logs;
        setWorkoutCount(logs.length);

        // Group by date for past workout rows
        const grouped: Record<string, PastWorkout> = {};
        logs.forEach((log: any) => {
          const date = log.sessionDate ?? log.createdAt?.split('T')[0] ?? 'Unknown';
          if (!grouped[date]) {
            grouped[date] = {
              id: date,
              date,
              time: new Date(log.createdAt ?? Date.now()).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }),
  ...
  const workoutsThisWeek = pastWorkouts.filter(w => {
    ...
    <View style={ls.container}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={ls.topBar}>
          ...
          <Text style={ls.topBarTitle}>Lloyd D</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={ls.statsSection}>
            <View style={ls.statCard}>
              <Text style={ls.statValue}>{workoutsThisWeek}/4 <Text style={ls.statValueSub}>days</Text></Text>
              <Text style={ls.statLabel}>WEEKLY GOAL</Text>
            </View>
            <View style={ls.statDivider} />
            <View style={ls.statCard}>
              <Text style={ls.statValue}>0 <Text style={ls.statValueSub}>weeks</Text></Text>
